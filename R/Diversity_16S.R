...
Diversity_16S <- function(x, R=999){
  ...
  for(i in 1:nsamples(x)){
    ...
    for(j in 1:R){
      temp <- rarefy_even_depth(temp.phy, verbose=FALSE, replace=TRUE)
      # Calculate frequencies
      temp <- data.frame(transform_sample_counts(temp, fun= function(x) x/sum(x))@otu_table)
    ...
    rich <- breakaway(temp,print=FALSE,plot=FALSE,answers=TRUE)
    DIV[i,3] <- rich$est
    DIV[i,4] <- rich$seest
    rich.chao <- chao1_bc(temp, print=FALSE, answers=TRUE)
