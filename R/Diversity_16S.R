...
#' Errors for D1 and D2 are calculated by bootstrapping. Currently the breakaway
#' method is not yet included because it fails for certain samples.
#' 
...
Diversity_16S <- function(x, R = 999) {
  ...
  DIV <- matrix(nrow = phyloseq::nsamples(x), ncol = 8)
  ...
  for (i in 1:phyloseq::nsamples(x)) {
    ...
    for (j in 1:R) {
      ...
      if (j == R) {
        ...
        DIV[i, 5] <- mean(temp.D1)
        DIV[i, 6] <- stats::sd(temp.D1)
        DIV[i, 7] <- mean(temp.D2)
        DIV[i, 8] <- stats::sd(temp.D2)
...
#     DIV[i, 4] <- rich$seest
    ...
    DIV[i, 3] <- rich.chao$est
    DIV[i, 4] <- rich.chao$seest
  }
  colnames(DIV) = c("D0", "sd.D0", "D0.chao", "sd.D0.chao", "D1", "sd.D1", "D2", 
                    "sd.D2")
